...
export function OrganizationsView() {
  ...
  const columns = useMemo<ColumnDef<Organization>[]>(() => [
    { accessorKey: "name", header: "Медицинская организация", cell: ({row}) => <Link prefetch={false} href={`/organizations/${row.original.id}`} className="font-semibold text-foreground hover:text-primary hover:underline">{row.original.name}</Link> },
