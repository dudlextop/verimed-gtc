...
export function AmountBar({ data }: { data: DistributionPoint[] }) {
  return <div className="h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical" margin={{ left: 8, right: 10 }}><CartesianGrid stroke={chartTheme.grid} strokeDasharray="3 4" horizontal={false}/><XAxis type="number" hide/><YAxis type="category" dataKey="name" width={120} tick={{...chartTick, fontSize: 11}} tickLine={false} axisLine={false}/><Tooltip contentStyle={chartTooltipStyle} cursor={{fill: "hsl(var(--surface-soft))"}} formatter={(value: number) => money(value)}/><Bar dataKey="amount" name="Сумма" fill={chartTheme.finance} radius={[0, 6, 6, 0]}/></BarChart></ResponsiveContainer></div>;
}
