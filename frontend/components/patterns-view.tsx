...
function MobilePatternCard({pattern, href}: {pattern: RecurringPattern; href: string}) { return <Link prefetch={false} href={href} className="block focus-visible:ring-2 focus-visible:ring-ring"><Card className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-2xl font-bold tabular-nums">{pattern.importance_score}<span className="ml-1 text-xs font-normal text-muted-foreground">из 100</span></p><div className="mt-1"><PriorityBadge level={pattern.importance_level}/></div></div><span className="text-sm font-semibold">{pattern.review_status}</span></div><h2 className="mt-4 font-bold">{pattern.name}</h2><p className="mt-1 text-sm text-muted-foreground">{pattern.main_organization ?? "Несколько организаций"}</p><div className="mt-4 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs text-muted-foreground">Финансовая значимость</p><p className="mt-1 font-mono font-bold tabular-nums">{money(pattern.financial_significance)}</p></div><StabilityBadge level={pattern.stability_level} score={pattern.stability_score}/></div></Card></Link>; }
