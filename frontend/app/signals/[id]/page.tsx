...
export default function SignalPage() {
  ...
  const patternLinks = useApi(() => api.signalPatterns(params.id), [params.id]);
  const history = useApi(() => api.signalDecisionHistory(params.id), [params.id]);
  const recurrence = useApi(() => api.signalRecurrenceHistory(params.id), [params.id]);
  ...
  if (state.loading || patternLinks.loading || history.loading || recurrence.loading) return <div className="page-shell"><PageLoading/></div>;
    ...
    <div className="mt-7 grid min-w-0 gap-6 xl:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-6">
        ...
        <Step id="decision" number="4" title="Решение специалиста">
          ...
          <h3 className="mt-6 font-bold">Последние действия</h3><div className="mt-4">{recentHistory ? <DecisionTimeline history={recentHistory} onRefine={(eventId) => { setRefineEventId(eventId); setCommentDraft(""); setNotice("Выберите уточнённое решение и добавьте комментарий."); }}/> : <ErrorState message={history.error ?? "История недоступна"} retry={() => void history.retry()}/>}</div>
