...
export default function PatternPage() {
  ...
  const graph = useApi(() => api.patternGraph(params.id), [params.id]);
  const timeline = useApi(() => api.patternTimeline(params.id), [params.id]);
  const signals = useApi(() => api.patternSignals(params.id), [params.id]);
  const history = useApi(() => api.patternDecisionHistory(params.id), [params.id]);
  const recurrence = useApi(() => api.patternRecurrenceHistory(params.id), [params.id]);
  ...
  if (detail.loading || graph.loading || timeline.loading || signals.loading || history.loading || recurrence.loading) return <div className="page-shell"><PageLoading/></div>;
  const error = detail.error || graph.error || timeline.error || signals.error;
  if (error || !detail.data || !graph.data || !timeline.data || !signals.data) return <div className="page-shell"><ErrorState message={error ?? "Ответ сервиса неполон"} retry={() => { void detail.retry(); void graph.retry(); void timeline.retry(); void signals.retry(); }}/></div>;
